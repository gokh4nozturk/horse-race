...
<template>
  <Card
    class="overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 text-white border-0 shadow-xl"
  >
    <CardHeader class="pb-4 border-b border-gray-700 relative">
      <div
    ...
    <CardContent class="p-0">
      ...
      <div v-else>
        ...
        <div class="flex justify-center p-6 pb-2">
          <div class="flex items-end gap-1 h-36">
            ...
            <div v-if="sortedResults.length > 1" class="flex flex-col items-center">
              <div
                class="w-16 h-16 rounded-full border-4 flex items-center justify-center mb-2 overflow-hidden"
                :style="{
              ...
              <div
                class="w-20 h-28 bg-gray-700 mt-auto flex items-center justify-center rounded-t-md"
              >
                <div class="text-4xl font-bold text-gray-300">2</div>
              </div>
            </div>
            ...
            <div v-if="sortedResults.length > 0" class="flex flex-col items-center z-10">
              <div
                class="w-20 h-20 rounded-full border-4 flex items-center justify-center mb-2 relative"
                :style="{
              ...
              <div
                class="w-24 h-36 bg-gradient-to-b from-amber-600 to-amber-700 mt-auto flex items-center justify-center rounded-t-md"
              >
            ...
            <div v-if="sortedResults.length > 2" class="flex flex-col items-center">
              <div
                class="w-14 h-14 rounded-full border-4 flex items-center justify-center mb-2"
                :style="{
              ...
              <div
                class="w-20 h-20 bg-gray-700 mt-auto flex items-center justify-center rounded-t-md"
              >
                <div class="text-4xl font-bold text-gray-300">3</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="sortedResults.length > 3" class="px-4 pb-4">
          <div class="text-sm text-gray-400 uppercase tracking-wider mb-2 mt-2 font-medium">
            Other Positions
          </div>

          <div class="space-y-2">
            <div
              v-for="result in sortedResults.slice(3)"
              :key="result.horseId"
              class="flex items-center p-3 rounded-md bg-gray-800/50 border-l-2"
              :style="{ borderColor: getHorseColor(result.horseId) }"
            >
              <div
                class="font-semibold w-8 h-8 rounded-full flex items-center justify-center bg-gray-700 text-sm"
              >
                {{ result.place }}
              </div>
