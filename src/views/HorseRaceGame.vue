  ...
  <div class="min-h-screen bg-background bg-cover bg-center bg-blend-overlay text-foreground" style="
    ...
    <main class="container mx-auto grid grid-cols-12 gap-5 p-4 pt-[calc(var(--spacing-header)+1rem)]">
        ...
        <div class="p-2 border-b border-border bg-card flex justify-between items-center">
          <h2 class="text-xl font-bold text-foreground">
            <span v-if="raceStore.currentRound">
              {{ t('game.round') }} {{ raceStore.currentRound.id }} - {{ raceStore.currentRound.distance }}m
            </span>
            <span v-else>{{ t('game.raceTrack') }}</span>
          </h2>
          <div class="flex items-center gap-2">
            ...
            <Badge v-else variant="secondary" class="uppercase">{{ t('game.ready') }}</Badge>
          </div>
        </div>
      ...
      <aside class="col-span-12 lg:col-span-3 flex flex-col gap-4">
        ...
        <div v-if="raceStore.schedule.length > 0"
          ...
          <h3 class="text-lg font-bold mb-3 flex items-center text-foreground">
            ...
            {{ t('game.championshipProgress') }}
          </h3>
          <div class="flex justify-between items-center mb-2">
            <span class="text-xs text-muted-foreground">{{ t('game.progress') }}</span>
            <span class="text-xs font-bold text-foreground">
              {{ t('game.roundOfTotal', { current: raceStore.currentRoundIndex + 1, total: raceStore.schedule.length })
              }}
            </span>
          </div>
      ...
      <section v-if="raceStore.isRoundCompleted && currentResult && !raceStore.isAllRoundsCompleted"
        ...
        <h3 class="text-lg font-bold mb-3 flex items-center text-foreground">
          ...
          {{ t('game.roundResults', { round: raceStore.currentRound?.id }) }}
        </h3>
      ...
      <div v-if="raceStore.isAllRoundsCompleted"
        ...
        <h2 class="text-2xl font-bold text-white mb-2 drop-shadow-md">
          {{ t('game.championshipComplete') }}
        </h2>
        <p class="text-white text-opacity-90">
          {{ t('game.congratsMessage') }}
        </p>
    ...
    <div v-if="isInitialized && raceStore.schedule.length === 0"
      ...
      <h2 class="text-2xl font-bold mb-3 text-foreground">
        {{ t('game.noRacesGenerated') }}
      </h2>
      <p class="text-muted-foreground mb-4">{{ t('game.generateToBegin') }}</p>
      <button @click="handleGenerate"
        ...
        {{ t('game.generateNewChampionship') }}
      </button>
    </div>
  </div>
</template>
